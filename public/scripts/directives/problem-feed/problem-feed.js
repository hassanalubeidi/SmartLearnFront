...
angular.module('smartLearnIoApp')
    ...
    .controller('problemFeedController', ['$scope', '$http', '$timeout', 'JsonAPI', 'Problem', 'Topic', 'apiBaseUrl', '$sce', 'Review',
        function($scope, $http, $timeout, JsonAPI, Problem, Topic, apiBaseUrl, $sce, Review) {
           ...
           }).then(function(response) { //Deals with grouping problems
           ...
           $scope.makeTestPaperURL = function(pagenum, paper) {
                   $('.testpaper.modal')
                       .modal('attach events', '.showTestPaperButton', 'show');
                   if (paper == undefined) {
                       paper = "http://www.ocr.org.uk/Images/61533-question-paper-unit-f325-equilibria-energetics-and-elements.pdf"
                   }
                   $scope.testPaperQuestionURL = paper + '#page=2&view=fitH,100'
           }

           $scope.trustSrc = function(src) { //fix for ng-src 
           ...
           $scope.reviewProblem = function(status, problem) {
               problem.reviewable = false
               var newReview = {
